        ...
        function onCreditAccountSearchInput() {
            debounce(() => renderCreditAccountsTable(), 350)();
        }
        ...
        async function viewApplication(id) {
            ...
            }).join('') : '<tr class="empty-row"><td colspan="2">No application attachments were submitted.</td></tr>';
            ...
            if (reviewableStatuses.includes(a.application_status)) {
                ...
                if (hasSufficientCapital) {
                    approveButtons = `<button class="btn btn-success" onclick="appAction(${a.application_id},'approve')">Approve</button>
                             <button class="btn btn-primary" onclick="appAction(${a.application_id},'approve_disburse')">Approve & Disburse</button>`;
                } else {
                    approveButtons = `<button class="btn btn-success" disabled style="opacity:0.5;cursor:not-allowed;">Approve</button>
                             <button class="btn btn-primary" disabled style="opacity:0.5;cursor:not-allowed;">Approve & Disburse</button>`;
                }
        ...
        let creditAccountSort = { field: 'client_name', direction: 'asc' };
        ...
        async function loadCreditAccounts() {
            ...
            try {
                const response = await fetch(API.clients + '?action=list');
                const result = await response.json();

                if (result.status !== 'success') {
                    tbody.innerHTML = `<tr class="empty-row"><td colspan="5">${escapeHtml(result.message || 'The credit accounts list is unavailable right now.')}</td></tr>`;
                    return;
                }

                const rows = result.data || [];
                if (!rows.length) {
                    tbody.innerHTML = '<tr class="empty-row"><td colspan="5">No credit accounts found.</td></tr>';
                    return;
                }

                // Calculate score changes for each client to determine status
                const clientsWithStatus = await Promise.all(rows.map(async (client) => {
                    try {
                        const scoreResponse = await fetch(API.clients + '?action=calculate_credit_score&client_id=' + client.client_id);
                        const scoreResult = await scoreResponse.json();
                        if (scoreResult.status === 'success') {
                            client.scoreCalculation = scoreResult.data;
                        }
                    } catch (e) {
                        // If calculation fails, just proceed without status
                    }
                    return client;
                }));

                creditAccountsData = clientsWithStatus;
                renderCreditAccountsTable();
            } catch (error) {
                tbody.innerHTML = `<tr class="empty-row"><td colspan="5">${escapeHtml(getRequestErrorMessage(error, 'Could not load credit accounts.'))}</td></tr>`;
            }
        }

        function renderCreditAccountsTable() {
            const tbody = document.getElementById('creditAccountsTbody');
            if (!tbody) return;

            const searchInput = document.getElementById('creditAccountSearch');
            const search = searchInput?.value?.trim() || '';

            let filteredData = creditAccountsData;

            // Apply search filter
            if (search) {
                const searchLower = search.toLowerCase();
                filteredData = filteredData.filter(client => {
                    const fullName = `${client.first_name || ''} ${client.middle_name || ''} ${client.last_name || ''} ${client.suffix || ''}`.toLowerCase();
                    const email = (client.email_address || '').toLowerCase();
                    const phone = (client.contact_number || '').toLowerCase();
            ...
            filteredData.sort((a, b) => {
                let aVal, bVal;
                switch (creditAccountSort.field) {
                    case 'client_name':
                        aVal = `${a.first_name || ''} ${a.middle_name || ''} ${a.last_name || ''} ${a.suffix || ''}`.toLowerCase();
                        bVal = `${b.first_name || ''} ${b.middle_name || ''} ${b.last_name || ''} ${b.suffix || ''}`.toLowerCase();
                        break;
                    case 'credit_score':
                        aVal = a.credit_score || 0;
                        bVal = b.credit_score || 0;
                        break;
                    case 'credit_limit':
                        aVal = parseFloat(a.credit_limit || 0);
                        bVal = parseFloat(b.credit_limit || 0);
                        break;
                    case 'status':
                        aVal = getStatusBadgeText(a);
                        bVal = getStatusBadgeText(b);
                        break;
                    default:
                        return 0;
                }
                if (aVal < bVal) return creditAccountSort.direction === 'asc' ? -1 : 1;
                if (aVal > bVal) return creditAccountSort.direction === 'asc' ? 1 : -1;
                return 0;
            });

            if (!filteredData.length) {
                tbody.innerHTML = '<tr class="empty-row"><td colspan="5">No credit accounts found matching your search.</td></tr>';
                return;
            }

            tbody.innerHTML = filteredData.map(client => {
                const fullName = `${client.first_name || ''} ${client.middle_name || ''} ${client.last_name || ''} ${client.suffix || ''}`.replace(/\s+/g, ' ').trim();
                const creditScore = client.credit_score || 0;
                const creditLimit = parseFloat(client.credit_limit || 0);

                // Determine status badge based on credit_limit vs baseline_limit.
                let statusBadge = '';
                if (client.scoreCalculation) {
                    const baseline = parseFloat(client.scoreCalculation.baseline_limit || 0);
                    const current = parseFloat(client.scoreCalculation.old_limit || creditLimit);
                    const netChange = client.scoreCalculation.net_change || 0;
                    const pendingChange = Math.abs(netChange) > 0;
                    const delta = current - baseline;

                    if (pendingChange && netChange > 0) {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#dcfce7;color:#166534;">Upgrade Pending</span>';
                    } else if (pendingChange && netChange < 0) {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#fee2e2;color:#991b1b;">Downgrade Pending</span>';
                    } else if (baseline > 0 && delta > 0.01) {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#dcfce7;color:#166534;">Growth</span>';
                    } else if (baseline > 0 && delta < -0.01) {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#fee2e2;color:#991b1b;">Downgraded</span>';
                    } else if (baseline > 0) {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#e5e7eb;color:#374151;">Baseline</span>';
                    }
                }

                // Fallback: if no calculation data, show "Active" if client has a limit, else "No Data"
                if (!statusBadge) {
                    if (creditLimit > 0) {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#dbeafe;color:#1e40af;">Active</span>';
                    } else {
                        statusBadge = '<span style="display:inline-block;padding:4px 8px;border-radius:4px;font-size:.75rem;font-weight:600;background:#f3f4f6;color:#6b7280;">No Data</span>';
                    }
                }

                return `<tr>
                <td style="font-weight:600;">${escapeHtml(fullName)}</td>
                <td><span style="font-weight:600;">${creditScore}</span></td>
                <td>${creditLimit > 0 ? '₱' + creditLimit.toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2}) : '<span style="color:var(--text-muted);">No limit</span>'}</td>
                <td>${statusBadge || '<span style="color:var(--text-muted);">-</span>'}</td>
                <td style="text-align:center;">
                    <button class="icon-btn table-icon-btn" onclick="viewCreditScoreDetails(${client.client_id})" title="View credit score details" aria-label="View credit score details">
            ...
            const sortIndicator = document.getElementById('sort-' + creditAccountSort.field);
            if (sortIndicator) {
                sortIndicator.textContent = creditAccountSort.direction === 'asc' ? '▲' : '▼';
            }
        }

        function getStatusBadgeText(client) {
            const creditLimit = parseFloat(client.credit_limit || 0);
            if (client.scoreCalculation) {
                const baseline = parseFloat(client.scoreCalculation.baseline_limit || 0);
                const current = parseFloat(client.scoreCalculation.old_limit || creditLimit);
                ...
                if (pendingChange && netChange > 0) return 'Upgrade Pending';
                if (pendingChange && netChange < 0) return 'Downgrade Pending';
                if (baseline > 0 && delta > 0.01) return 'Growth';
                if (baseline > 0 && delta < -0.01) return 'Downgraded';
                if (baseline > 0) return 'Baseline';
            }
            if (creditLimit > 0) return 'Active';
            return 'No Data';
        }

        function sortCreditAccounts(field) {
            if (creditAccountSort.field === field) {
                creditAccountSort.direction = creditAccountSort.direction === 'asc' ? 'desc' : 'asc';
            } else {
                creditAccountSort.field = field;
                creditAccountSort.direction = 'asc';
            }
            renderCreditAccountsTable();
        }
        ...
        async function viewCreditScoreDetails(clientId) {
            openModal('creditScoreModal');
            document.getElementById('creditScoreModalBody').innerHTML = '<div style="text-align:center;padding:32px;"><span class="spinner"></span></div>';
            document.getElementById('creditScoreModalFooter').innerHTML = `
                <button class="btn btn-outline" onclick="closeModal('creditScoreModal')">Close</button>
            `;

            try {
                const response = await fetch(API.clients + '?action=calculate_credit_score&client_id=' + clientId);
                const result = await response.json();

                if (result.status !== 'success') {
                    document.getElementById('creditScoreModalBody').innerHTML = `<p style="color:#ef4444;">${escapeHtml(result.message || 'Could not load credit score details.')}</p>`;
                    return;
                }
                ...
                document.getElementById('creditScoreModalTitle').textContent = 'Credit Evaluation';

                const scoreChanged = data.net_change !== 0;
                const limitChanged = data.rules_triggered;
                const hasActiveOverdue = metrics.has_active_overdue;
                const latePayments = metrics.late_payments_in_period || 0;
                const successfulCycles = metrics.successful_cycles || 0;
                const maxOverdueDays = metrics.max_overdue_days || 0;

                // Build narrative based on data
                let statusNarrative = '';
                let conditionsMet = [];
                let conditionsMissing = [];
                let nextSteps = [];

                // Analyze bonuses
                if (data.bonuses && data.bonuses.length > 0) {
                    data.bonuses.forEach(b => {
                        if (b.reason.includes('successful repayment')) {
                            conditionsMet.push(`Completed ${successfulCycles} on-time repayment cycles`);
                        } else if (b.reason.includes('no active overdue')) {
                            conditionsMet.push('No active overdue balances');
                        } else if (b.reason.includes('late payments')) {
                            conditionsMet.push('No late payments in review period');
                        }
                    });
                }
                ...
                if (data.penalties && data.penalties.length > 0) {
                    data.penalties.forEach(p => {
                        if (p.reason.includes('overdue')) {
                            conditionsMissing.push('Active overdue balance detected');
                            nextSteps.push('Clear overdue balance to restore standing');
                        } else if (p.reason.includes('late payment')) {
                            conditionsMissing.push(`${latePayments} late payment(s) in review period`);
                            nextSteps.push('Make payments on time for 90 days to improve score');
                        } else if (p.reason.includes('overdue days')) {
                            conditionsMissing.push(`Maximum overdue days: ${maxOverdueDays}`);
                            nextSteps.push('Reduce overdue days to improve payment history');
                        }
                    });
                }

                // If no changes, explain why
                if (!scoreChanged && !limitChanged) {
                    if (hasActiveOverdue) {
                        conditionsMissing.push('Active overdue balance blocks progression');
                        nextSteps.push('Clear overdue balance to unlock limit increases');
                    } else if (latePayments > 0) {
                        conditionsMissing.push('Recent late payments affecting score');
                        nextSteps.push('Maintain on-time payments for 90 days to rebuild standing');
                    } else if (successfulCycles < 3) {
                        conditionsMissing.push(`Need more on-time repayment cycles (currently ${successfulCycles})`);
                        nextSteps.push('Complete 3+ on-time repayment cycles to qualify for limit increase');
                    } else {
                        conditionsMet.push('Account in good standing');
                        nextSteps.push('Continue making on-time payments to maintain current limit');
                    }
                }
                ...
                if (conditionsMet.length > 0) {
                    conditionsHtml += `
                        <div style="margin-bottom:12px;">
                            <div style="font-size:.75rem;font-weight:600;color:#10b981;margin-bottom:6px;">✓ CONDITIONS MET</div>
                            ${conditionsMet.map(c => `<div style="font-size:.85rem;padding:4px 0;color:var(--text);">${escapeHtml(c)}</div>`).join('')}
                        </div>
                    `;
                }
                if (conditionsMissing.length > 0) {
                    conditionsHtml += `
                        <div style="margin-bottom:12px;">
                            <div style="font-size:.75rem;font-weight:600;color:#ef4444;margin-bottom:6px;">✗ CONDITIONS BLOCKING PROGRESSION</div>
                            ${conditionsMissing.map(c => `<div style="font-size:.85rem;padding:4px 0;color:var(--text);">${escapeHtml(c)}</div>`).join('')}
                        </div>
                    `;
                }
                ...
                if (nextSteps.length > 0) {
                    nextStepsHtml = `
                        <div style="margin-top:20px;padding:16px;background:color-mix(in srgb, var(--brand), transparent 90%);border-radius:8px;border-left:3px solid var(--brand);">
                            <div style="font-size:.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;">Next Steps</div>
                            ${nextSteps.map(s => `<div style="font-size:.85rem;padding:4px 0;color:var(--text);">• ${escapeHtml(s)}</div>`).join('')}
                        </div>
                    `;
                }

                document.getElementById('creditScoreModalBody').innerHTML = `
        <div style="margin-bottom:24px;">
            <div style="font-size:.85rem;color:var(--text-muted);margin-bottom:4px;">Current Credit Score</div>
            <div style="font-size:2rem;font-weight:700;">${data.old_score}</div>
            ${scoreChanged ? `
                <div style="margin-top:8px;font-size:.85rem;">
                    <span style="color:${data.net_change >= 0 ? '#10b981' : '#ef4444'};font-weight:600;">
                        ${data.net_change >= 0 ? 'Increased' : 'Decreased'} by ${Math.abs(data.net_change)} points
                    </span>
                    <span style="color:var(--text-muted);"> → New score: ${data.new_score}</span>
                </div>
            ` : `
                <div style="margin-top:8px;font-size:.85rem;color:var(--text-muted);">Score unchanged in this evaluation</div>
            `}
        </div>

        <div style="margin-bottom:24px;">
            <div style="font-size:.85rem;color:var(--text-muted);margin-bottom:4px;">Credit Limit</div>
            <div style="font-size:1.5rem;font-weight:700;">
                ₱${(data.rules_triggered ? data.new_limit : data.old_limit).toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
            </div>
            ${limitChanged ? `
                <div style="margin-top:8px;font-size:.85rem;">
                    <span style="color:${data.new_limit > data.old_limit ? '#10b981' : '#ef4444'};font-weight:600;">
                        ${data.new_limit > data.old_limit ? 'Increased' : 'Decreased'} from ₱${data.old_limit.toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                    </span>
                </div>
            ` : `
                <div style="margin-top:8px;font-size:.85rem;color:var(--text-muted);">Limit unchanged — conditions for adjustment not met</div>
            `}
        </div>

        <div style="margin-bottom:20px;padding:16px;background:var(--border-bg);border-radius:8px;">
            <div style="font-size:.85rem;font-weight:600;margin-bottom:12px;">Evaluation Summary</div>
            ${conditionsHtml || '<div style="font-size:.85rem;color:var(--text-muted);">No specific conditions evaluated in this cycle</div>'}
        </div>

        ${nextStepsHtml}
                `;
            } catch (error) {
                document.getElementById('creditScoreModalBody').innerHTML = `<p style="color:#ef4444;">${escapeHtml(getRequestErrorMessage(error, 'Could not load credit score details.'))}</p>`;
            }
        }
