-- MicroFin Definitive Fresh Reset Script (Verbatim Schema)
-- Generated for gleomir28@gmail.com
-- This script contains 100% verbatim table structures from Dump20260420.sql
...
CREATE TABLE `loan_products` (
  ...
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `early_settlement_fee_type` enum('remaining_balance_pct','remaining_principal_pct','fixed','rebate_only','rebate_plus_pct','rebate_plus_fixed','no_early_settlement_changes') COLLATE utf8mb4_unicode_ci DEFAULT 'no_early_settlement_changes',
...
DROP TABLE IF EXISTS `tenant_capital`;
CREATE TABLE `tenant_capital` (
  `capital_id` int NOT NULL AUTO_INCREMENT,
  `tenant_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  `available_balance` decimal(15,2) NOT NULL DEFAULT '0.00' COMMENT 'Current funds available for disbursement',
  `reserved_amount` decimal(15,2) NOT NULL DEFAULT '0.00' COMMENT 'Amount reserved for approved but not yet disbursed applications',
  `total_disbursed` decimal(15,2) NOT NULL DEFAULT '0.00' COMMENT 'Lifetime total of all loan releases',
  `total_replenished` decimal(15,2) NOT NULL DEFAULT '0.00' COMMENT 'Lifetime total of all repayments credited back',
  `low_balance_threshold` decimal(15,2) DEFAULT '0.00' COMMENT 'Warn admin when available_balance drops below this',
  ...
  KEY `updated_by` (`updated_by`),
  CONSTRAINT `tenant_capital_ibfk_1` FOREIGN KEY (`tenant_id`) REFERENCES `tenants` (`tenant_id`) ON DELETE CASCADE,
  CONSTRAINT `tenant_capital_ibfk_2` FOREIGN KEY (`updated_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

DROP TABLE IF EXISTS `tenant_disbursement_methods`;
CREATE TABLE `tenant_disbursement_methods` (
  `method_id` int NOT NULL AUTO_INCREMENT,
  `tenant_id` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  `method_name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'e.g. GCash, BPI Bank Transfer, Over-the-Counter',
  `method_type` enum('E-Wallet','Bank Transfer','OTC','Account Credit','Cash') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Cash',
  `account_name` varchar(150) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Account holder / business name',
  `account_number` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Wallet/account number (masked in UI)',
  `provider` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Bank or e-wallet provider name',
  `is_enabled` tinyint(1) NOT NULL DEFAULT '1',
  `is_default` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'Default method pre-selected on loan release',
  ...
  CONSTRAINT `tenant_disbursement_methods_ibfk_1` FOREIGN KEY (`tenant_id`) REFERENCES `tenants` (`tenant_id`) ON DELETE CASCADE,
  CONSTRAINT `tenant_disbursement_methods_ibfk_2` FOREIGN KEY (`created_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Document Types (Full 41 items verbatim)
