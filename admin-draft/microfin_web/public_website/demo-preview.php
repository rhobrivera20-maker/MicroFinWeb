...
// Force guest mode for demo preview
$is_guest_mode = true;
...
$pageData = [
    'company_name' => 'Demo Microfinance',
    'short_name' => 'Demo',
    'primary_color' => '#2563eb',
    'border_color' => '#e2e8f0',
    'text_heading_color' => '#0f172a',
    'text_body_color' => '#64748b',
    'btn_bg_color' => '#2563eb',
    'btn_text_color' => '#ffffff',
    'border_radius' => '16',
    'shadow_intensity' => '0.1',
    'logo_url' => '',
    'hero_image' => 'https://images.unsplash.com/photo-1556740714-a8395b3bf30f?auto=format&fit=crop&w=600&h=800&q=80',
    'hero_title' => 'Empowering Your Financial Future',
    'hero_subtitle' => 'Get flexible loans with fast approval and transparent terms.',
    'hero_badge_text' => 'Verified Partner',
    'about_body' => 'We believe in empowering our community through accessible financial tools.',
    'download_description' => 'Track your loans, submit applications, receive notifications...',
    'contact_address' => '123 Finance Ave, Business District',
    'contact_phone' => '0900-123-4567',
    'contact_email' => 'hello@demomicrofin.os',
    'contact_hours' => 'Mon-Fri: 8AM - 5PM',
    'footer_desc' => 'Your trusted microfinance partner.',
    'services' => [
        ['icon' => 'person', 'title' => 'Personal Loan', 'description' => 'Fast approval for your personal needs.'],
        ['icon' => 'home', 'title' => 'Home Loan', 'description' => 'Build your dream home with flexible terms.'],
        ['icon' => 'business', 'title' => 'Business Loan', 'description' => 'Grow your business with our support.']
    ],
    'stats' => [
...
// ==========================================
$tenant_plan_tier = 'Enterprise'; // Show all templates in demo mode
...
if (is_dir($templates_dir)) {
    foreach (scandir($templates_dir) as $file) {
        if (pathinfo($file, PATHINFO_EXTENSION) === 'php') {
            ...
            // Only allow templates 1, 2, 4, 5, 9
...
// Preview template from URL
$selected_template = $_GET['tpl'] ?? $available_templates[0];
...
$save_endpoint = builder_app_base_path() . '/public_website/demo-preview.php';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Website Builder Demo — <?php echo $e($company_name); ?></title>
    ...
    <style>
        ...
        /* Scope template CSS variables to canvas only */
        .editor-canvas {
            --brand: #2563eb;
            --border-clr: #e2e8f0;
            --radius: 16px;
...
<body>
    <div class="editor-sidebar">
        <a href="index.php" class="back-link">
            <span class="material-symbols-rounded" style="font-size:18px;">arrow_back</span> Back to Home
        </a>
        ...
        <div class="d-flex gap-2 mt-3">
            ...
            <a href="index.php" class="btn btn-outline-secondary py-2" title="Back to Home">
    ...
    <div class="editor-canvas" id="canvasArea">
        ...
        <div id="templateLoader" style="display:none; text-align:center; padding:4rem;">
    ...
    <script>
        ...
        document.getElementById('inp_color').addEventListener('input', (e) => canvas.style.setProperty('--brand', e.target.value));
        document.getElementById('inp_border').addEventListener('input', (e) => canvas.style.setProperty('--border-clr', e.target.value));
        document.getElementById('inp_radius').addEventListener('input', (e) => canvas.style.setProperty('--radius', e.target.value + 'px'));
        document.getElementById('inp_shadow').addEventListener('input', (e) => canvas.style.setProperty('--shadow', `0 8px 24px rgba(0,0,0,${e.target.value})`));

        // Typography & Button Live Preview
        document.getElementById('inp_text_heading').addEventListener('input', (e) => canvas.style.setProperty('--text-heading', e.target.value));
        document.getElementById('inp_text_body').addEventListener('input', (e) => canvas.style.setProperty('--text-body', e.target.value));
        document.getElementById('inp_btn_bg').addEventListener('input', (e) => canvas.style.setProperty('--btn-bg', e.target.value));
        document.getElementById('inp_btn_text').addEventListener('input', (e) => canvas.style.setProperty('--btn-text', e.target.value));
        ...
        iconList.forEach(icon => {
            ...
            el.onclick = () => {
                if (currentIconTarget) { currentIconTarget.innerText = icon; }
        ...
        function reinitializeTemplateEvents() {
            ...
            // Disable login and download buttons in demo mode
            const canvas = document.getElementById('canvasArea');
            if (canvas) {
                canvas.querySelectorAll('a[href*="login"], a[href*="download"]').forEach(btn => {
        ...
        window.addServiceCard = function() {
            ...
            if (addCol) addCol.insertAdjacentHTML('beforebegin', blueprint);
        };
        ...
        // Disable all login and download buttons in demo mode
        window.addEventListener('load', function() {
            const canvas = document.getElementById('canvasArea');
            if (canvas) {
                const buttons = canvas.querySelectorAll('a[href*="login"], a[href*="get-app"]');
                buttons.forEach(btn => {
                    btn.style.pointerEvents = 'none';
                    btn.style.opacity = '0.5';
                    btn.style.cursor = 'not-allowed';
                    btn.setAttribute('title', 'Disabled in demo mode');
                });
            }
        });
    </script>
